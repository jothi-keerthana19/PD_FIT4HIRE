...
def analyze_resume():
    """
    Endpoint to analyze a resume for job fit or hiring
    Expects: 
    - file: Resume file
    - role: Job role
    - skills: Optional comma-separated custom skills
    - mode: 'fit' or 'hire' (default: 'fit')
    """
    try:
        ...
        # Validate job role
        valid_roles = list(Fit.JOB_ROLES.keys())
        if role not in valid_roles:
            # Fallback to default role if invalid
